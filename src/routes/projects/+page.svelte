<script lang="ts">
  import { projects } from "$lib/components/force-graph/data.js";
...
<div class="w-full flex flex-col items-center bg-slate-100 py-4 font-mono">
  <div class="text-2xl md:text-4xl found-mono">
    Projects
  </div>

  <!-- search bar -->
  <div
    class="w-full flex flex-col gap-2 items-center my-4 md:my-6 relative inline-block"
  >
    <input
    ...
    <div class="w-1/4 flex flex-row flex-wrap gap-1">
      ...
      <div class="dropdown">
        ...
        <ul
          tabindex="0"
          class="dropdown-content menu bg-slate-200 rounded-md z-1 shadow-sm mt-1"
        >
          <li>
            <input 
              bind:value={toolSearchFilter}
              placeholder="Search"
              class='px-2'
            />
  ...
  <!-- grid -->
  <div
    class="w-2/3 grid grid-cols-1 lg:grid-cols-3 auto-rows-fr gap-2 font-mono"
  >
    {#each projects
      ...
      <a href="{base}{project.url}">
        ...
        >
          <div class="flex flex-row">
            <div
              class="text-xl md:text-3xl font-bold group-hover:underline pb-2"
            >
              {project.name}
            </div>
